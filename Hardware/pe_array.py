...
class PE_Array:
    ...
    def __init__(
        ...
        self.pe_array_area  = self.pe_area * self.total_pe_count
    ...
    def _init_model_profiler(self, model_name, context_length: int=256, is_generation: bool=False):
        ...
        for name, shape in layer_config.items():
            ...
            if is_generation: # generation
                input_dim[name]  = [self.pe_dp_size, shape[1]]
                output_dim[name] = [self.pe_dp_size, shape[0]]
            else:
                input_dim[name]  = [context_length*self.pe_dp_size, shape[1]]
                output_dim[name] = [context_length*self.pe_dp_size, shape[0]]
