...
class Accelerator(PE_Array):
    ...
    def _calc_tile_fc(self, w_dim, o_dim):
        ...
        tile_in_channel  = math.ceil(cin)
        tile_cout        = math.ceil(cout / num_pe_col)
        tile_token       = math.ceil(num_token / num_pe_row)
    ...
    def _calc_dram_cycle(self):
        ...
        dram_bandwidth = self.dram.rw_bw # HBM
        ...
        for name in self.layer_name_list:
            ...
            cycle_layer_dram = cycle_dram_load_w
    ...
    def _init_mem(self):
        if self.is_bit_serial:
            w_bandwidth = 64* math.ceil(self.w_prec / 4) * 4 * 16 / 2
        else:
            w_bandwidth = 64 * math.ceil(self.w_prec / 4) * 4 * 16
        w_sram_bank = 8 * 128
        w_sram_config = {
            'technology': 0.028,
            'mem_type': 'ram', 
        ...
        if self.is_bit_serial:
            i_bandwidth = 64 * self.i_prec * 128 / 2
        else:
            i_bandwidth = 64 * self.i_prec * 128
        i_sram_bank = 8*128
        i_sram_config = {
            'technology': 0.028,
            'mem_type': 'ram', 
        ...
        wr_cost = dram_rw_bw / (64 * 8) * 1200 *4 
